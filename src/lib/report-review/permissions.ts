...
const resolvedStatuses: ReviewFindingStatus[] = ["Accepted Explanation", "Corrected", "Dismissed", "Resolved"];
...
export function canViewReportFinding(context: ReportReviewVisibilityContext, finding: ReviewFinding) {
  ...
  if (user.role === "client_user") return finding.visibility.includes("client");
