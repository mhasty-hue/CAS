...
insert into public.order_import_templates (id, organization_id, client_id, name, source_type, column_mappings, field_rules, active, created_by) values
  ('99600000-0000-4000-8000-000000000001', '11111111-1111-1111-1111-111111111111', 'c0000000-0000-4000-8000-000000000001', 'HarborPoint CSV export', 'CSV', '[{"sourceLabel":"Borrower Full Name","targetKey":"borrower"},{"sourceLabel":"Subject Address","targetKey":"property_address"},{"sourceLabel":"Appraisal Form","targetKey":"product_type"},{"sourceLabel":"Due Date","targetKey":"due_date"}]', '{"duplicateKeys":["amc_file_number","borrower","property_address"]}', true, null)
on conflict (organization_id, name) do update set client_id = excluded.client_id, source_type = excluded.source_type, column_mappings = excluded.column_mappings, field_rules = excluded.field_rules, active = excluded.active, updated_at = now();

insert into public.order_import_sessions (id, organization_id, order_id, import_template_id, source_file_name, source_content_type, source_size_bytes, source_type, provider, extracted_fields, mapping_decisions, duplicate_candidates, validation_warnings, confidence, status, created_by, confirmed_at) values
  ('99700000-0000-4000-8000-000000000001', '11111111-1111-1111-1111-111111111111', 'e0000000-0000-4000-8000-000000000006', '99600000-0000-4000-8000-000000000001', 'harborpoint-order-export.csv', 'text/csv', 18420, 'CSV', 'demo', '[{"key":"client","value":"HarborPoint Lending","confidence":0.91},{"key":"borrower","value":"Morgan Ellis","confidence":0.84},{"key":"property_address","value":"2218 Briarwood Crossing","confidence":0.86},{"key":"product_type","value":"1004 URAR","confidence":0.86}]', '[{"key":"client","decision":"accept"},{"key":"borrower","decision":"accept"},{"key":"property_address","decision":"accept"},{"key":"product_type","decision":"accept"}]', '[]', '["PDF extraction remains demo-only until an external processor is configured."]', 0.8675, 'Confirmed', null, '2026-07-09 13:06:00-04')
on conflict (id) do update set source_file_name = excluded.source_file_name, extracted_fields = excluded.extracted_fields, mapping_decisions = excluded.mapping_decisions, duplicate_candidates = excluded.duplicate_candidates, validation_warnings = excluded.validation_warnings, confidence = excluded.confidence, status = excluded.status, confirmed_at = excluded.confirmed_at;
